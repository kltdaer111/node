...
const statistics_gen = require('./statistics_gen');
...
var task_table = ['栏名', '达成人数', '相对达成率', '绝对达成率', '离开人数', '次日留存', '留存率'];
...
Worker.prototype.start_work = function(type, col_table){
	this.type = type;
	if(col_table === undefined){
		this.col_table = task_table;
	}
	else{
		this.col_table = col_table;
	}
	this.con_glog = mysql.createConnection(glog_dbconfig);
	this.con_log = mysql.createConnection(log_dbconfig);
...
Worker.prototype.do_with_results = function(){
	...
	this.con_log.query("INSERT INTO daily_statistics_log(type,data,date) VALUES(?,?,FROM_UNIXTIME(?));", [this.type, str, timestamp_tdby_begin], function(err, results, fields){
		if(err){
			throw err;
		}
		console.log('OK');
...
Worker.prototype.get_mysql_data_for_processor = function(){
	...
	const self = this;
	this.con_glog.query('SELECT * FROM account_log WHERE UNIX_TIMESTAMP(create_date)=? AND (source_type=1562 OR source_type=1560);', [timestamp_tdby_begin], function(error, results, fields){
	...
	this.con_log.query('SELECT * FROM login_log WHERE UNIX_TIMESTAMP(create_time)>=? AND UNIX_TIMESTAMP(create_time)<?;', [timestamp_yesterday_begin, timestamp_today_begin], function(error, results, fields){
		if(error){
			throw error;
		}
	...
	switch(this.type){
		case 'task':{
			this.con_log.query('SELECT * FROM task_log WHERE task_type=1 AND op_type=702 AND UNIX_TIMESTAMP(create_time)>=? AND UNIX_TIMESTAMP(create_time)<?;', [timestamp_tdby_begin, timestamp_yesterday_begin], function(error, results, fields){
		...
		break;
		case 'level':{
			this.con_log.query('SELECT * FROM role_exp_level_log WHERE old_level!=new_level AND source_param != 0 AND UNIX_TIMESTAMP(create_time)>=? AND UNIX_TIMESTAMP(create_time)<?;', [timestamp_tdby_begin, timestamp_yesterday_begin], function(error, results, fields){
				//console.log(results[1]);
				if(error){
					throw error;
				}
		...
		break;
		case 'duration':{
			this.con_log.query('SELECT role_uid,sum(online_time) as duration FROM login_log WHERE UNIX_TIMESTAMP(create_time)>=? AND UNIX_TIMESTAMP(create_time)<? GROUP BY role_uid;', [timestamp_tdby_begin, timestamp_yesterday_begin], function(error, results, fields){
				//console.log(results[1]);
				if(error){
					throw error;
				}
...
con_glog = mysql.createConnection(glog_dbconfig);

var worker4 = new statistics_gen.NewUserGen(con_glog, timestamp_yesterday_begin);
var worker5 = new statistics_gen.ActiveUserGen(con_glog, timestamp_yesterday_begin);
var worker6 = new statistics_gen.RemainUser(con_glog, timestamp_yesterday_begin);
var worker7 = new statistics_gen.ComeBackUser(con_glog, timestamp_yesterday_begin - 4 * 24 * 60 * 60, timestamp_yesterday_begin);
